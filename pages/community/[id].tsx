...
const CommunityPostDetail: NextPage = () => {
  ...
  const onWonderClick = () => {
    ...
    if(!loading)  wonder({});// prevent race condition
  }
  ...
  return (
    <Layout canGoBack seoTitle="CommunityDetail">
      <div>
        ...
        <div>
          ...
          <div className="flex px-4 space-x-5 mt-3 text-gray-700 py-2.5 border-t border-b-[2px]  w-full">
            <button 
              ...
              className={cls("flex space-x-2 items-center text-sm", 
              data?.isWondering ? 
              "text-teal-400" : ""
              )}>
              <svg
          ...
          {data?.post?.answers?.map((answer) => <div key={answer.id} className="flex items-start space-x-3">
            <div className="w-8 h-8 bg-slate-200 rounded-full" />
              <div>
                ...
                <p key={answer.answer} className="text-gray-700 mt-2"> {answer.answer} </p>
              </div>
          </div>)}
        </div>
